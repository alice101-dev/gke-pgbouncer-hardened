...
[![CI](https://github.com/alice101-dev/gke-pgbouncer-hardened/actions/workflows/ci.yml/badge.svg)](https://github.com/alice101-dev/gke-pgbouncer-hardened/actions/workflows/ci.yml)

A production-grade **`PgBouncer`** deployment for GKE with zero secrets in Git, **`Workload Identity`**, **`External Secrets Operator (ESO)` for `GCP Secret Manager`**, **`SCRAM-SHA-256`** authentication, enforced TLS to Cloud SQL, verified supply chain integrity, and a fully hardened runtime security context.
...
- **Zero Secrets in Git:** Sensitive configuration details (SCRAM password verifiers and the Cloud SQL server CA certificate) are managed securely in GCP Secret Manager. The backend host IP lives in the plain ConfigMap — it is routing metadata, not a secret.
...
- **Secure Runtime Contexts:** Pods run as non-root with a high UID/GID (`10070`, avoiding host user collisions), enforce a read-only root filesystem, drop all Linux capabilities, skip service-account token mounts, and implement `RuntimeDefault` seccomp and AppArmor profiles (Kubernetes 1.30+).
- **Default-Deny NetworkPolicy:** Ingress is limited to pods labelled `pgbouncer-client: "true"` on port 5432; egress is limited to the Cloud SQL private IP and DNS.
...
- **Least Privilege (Non-Root):** Container execution runs under UID/GID `10070` (`pgbouncer`), a high UID that cannot collide with any host system user and cannot gain root access on the node.
- **No Kubernetes API Access:** `automountServiceAccountToken: false` — PgBouncer never talks to the K8s API, so no token is mounted for an attacker to steal.
- **Network Segmentation:** A default-deny `NetworkPolicy` allows only labelled client pods in on 5432 and only the Cloud SQL private IP (plus DNS) out.
...
# 5. Apply the NetworkPolicy (default-deny; clients need the pgbouncer-client=true label)
kubectl apply -f networkpolicy-pgbouncer.yaml
...
With the **2 application users** configured in `userlist.txt`, a `default_pool_size` of **40**, and **2 replicas**, the total maximum connections from PgBouncer to your PostgreSQL database can reach:
```text
2 * 40 * 2 = 160 connections
```
...
```bash
kubeconform -strict -summary -ignore-missing-schemas ./*.yaml   # manifest schema validation
checkov -d . --framework kubernetes,dockerfile                  # static security analysis
```

Checkov: **0 failed** across 160+ Kubernetes and Dockerfile policies, with two
documented skips (image digest pinning is deferred to the README build step;
container health is owned by Kubernetes probes rather than a Docker
`HEALTHCHECK`).
